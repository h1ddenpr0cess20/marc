 ...
 *
 * A task that settles is told to the model as it lands, so Marc says what
...
function chipState() {
  if (!session.connected || !session.muted) return marc.state;
  return marc.state === 'listening' || marc.state === 'idle' ? 'muted' : marc.state;
}
