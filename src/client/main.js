 ...
 *
 * Four pieces that don't know about each other: Marc (geometry), the session
 * (transport), the HUD (what you read) and the controls (what you press). This
 * file is the one place that knows a `level` event should become a squash and
...
import { createEggBuddy } from './egg/index.js';
...
/* --- session → Marc + HUD -------------------------------------------------
   ...
   sound; Marc folds both into the same energy. */

session.on('state', (state) => {
  // A new turn starts here: the previous answer clears as Marc thinks.
  if (state === 'thinking') hud.clearCaption();
  marc.setState(state);
  hud.setState(marc.state);
  controls.sync();
});
...
session.on('error', ({ message }) => {
  ...
  hud.setState(marc.state); // a failed dial never leaves 'idle', so no 'state' clears the chip
  controls.sync();
});
