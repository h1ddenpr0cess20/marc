...
/* There is no voices endpoint to ask, so this is the published set, filtered to
   the male ones — Marc has one voice range and the picker has no business
   offering to change it. `cedar` leads, and is therefore the default: it is
   realtime-native and the most naturalistic of them. `ballad` has a drier lift;
   `ash`, `echo` and `verse` predate cedar and read flatter. Left out as female
   or deliberately neutral: `marin`, `coral`, `sage`, `shimmer`, `alloy`.

   An unrecognised OPENAI_VOICE is still honoured and shows up in the picker —
   the list here goes stale, the API doesn't, and an operator naming a voice by
   hand has already said what they want. */
