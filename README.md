...
Marc has no tools beyond memory. He answers from what the model already knows: no web
search, no retrieval. Ask him about this morning and he should say he
...
The log is a record. Memory is the part Marc actually carries into the next
call: a short list of details, kept in `localStorage` under `marc.memory.v1`
and appended to the persona as a labelled block when the secret is minted.

Ask him to remember something and he calls `remember`; ask him to forget
it and he calls `forget`, which drops every stored line matching the keyword.
