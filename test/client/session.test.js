...
describe('createVoiceSession', () => {
  ...
  describe('a live call', () => {
    ...
    it('routes server events through to Marc\'s vocabulary', () => {
    ...
    it('keeps a transcript of both sides', () => {
      ...
      peer().channel.deliver({ type: 'response.output_text.delta', delta: 'An egg!' });
      peer().channel.deliver({ type: 'response.done', response: {} });

      assert.deepEqual(session.messages, [
        { role: 'user', content: 'what are you?' },
        { role: 'assistant', content: 'An egg!' },
      ]);
    });
    ...
    describe('typed input', () => {
      it('sends the item and asks for a response', () => {
        session.send('  hello Marc  ');
        assert.deepEqual(peer().channel.sent, [
          {
            type: 'conversation.item.create',
            item: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'hello Marc' }] },
          },
          { type: 'response.create' },
        ]);
        assert.equal(session.state, 'thinking');
        assert.deepEqual(session.messages, [{ role: 'user', content: 'hello Marc' }]);
      });
    ...
    describe('cancel', () => {
      it('does nothing when Marc is not talking', () => {
        session.cancel();
        assert.deepEqual(peer().channel.sent, []);
      });
  ...
  describe('stopping', () => {
    ...
    it('settles Marc: level zero, state idle', () => {
      session.stop();
